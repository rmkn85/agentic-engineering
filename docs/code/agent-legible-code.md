...
> **same required behavior + same quality bar → less code/context to localize, mentally model, change, validate, and debug**
...
## Primary invariant: a weak reader can build a bounded mental model

“Analyzable” in an agentic codebase does **not** mean solving the halting problem, proving every property, or spending an arbitrarily large reasoning budget until the model eventually works it out.

For a normal code unit, a fresh **weaker, reasoning-limited, context-limited coding model** should be able to read the unit plus a small amount of explicit surrounding contract/context and form a sufficiently accurate operational model of it.

It should be able to answer questions such as:

- what inputs and preconditions matter?
- what paths/branches can execute?
- what outputs/results are produced for representative normal and edge cases?
- what state can be read or changed?
- what external effects occur?
- what can fail, and how is failure propagated?
- which dependencies actually influence the result?
- what work is bounded, retried, asynchronous, or potentially non-terminating?
- which invariant would be violated if this code were wrong?

The goal is **reliable bounded prediction**, not formal proof. The reader may approximate uncommon runtime details, but it should not need to recursively inspect half the repository merely to understand ordinary behavior.

If a weaker model cannot form this mental model without repeated searches, extra files, long chain-of-thought-like reasoning, or trial execution, treat that as a design smell even when a stronger model can eventually solve it.

This is the most direct agent-era analogue of safety-critical “analyzability.” Static analyzers remain valuable, but they are supporting machinery; the primary consumer here is the next language-model context.
...
## Apply this from the first line and at every touch

Agent legibility should not be a cleanup phase reserved for mature code. Each code-writing or code-changing operation should preserve or improve the local mental-model boundary.

Before writing or changing a unit:

1. identify the smallest context that *should* be sufficient to understand its behavior;
2. identify the contract/state/dependencies that must be explicit inside that boundary;
3. avoid introducing hidden coupling that forces a future reader to widen the boundary.

After touching it, ask:

> **Could a fresh weaker coding model, given this unit and its immediate declared contracts, predict its representative behavior without repository archaeology?**

If not, first simplify the local design or expose the missing contract/state/dependency. Do not reflexively add a long explanatory comment that permanently compensates for avoidable complexity.

Do not turn this into mandatory opportunistic rewrites of unrelated legacy code. The rule is: **leave touched code no harder to model than before, and cheaply improve obvious local obstacles when doing so is low-risk.**
...
**Decision rule:** minimize the smallest sufficient context set for representative behavior and changes, not line count in isolation.
...
A dependency graph an indexer can reconstruct cheaply is also easier for an agent to retrieve selectively. More importantly, a reader should be able to tell from the unit which dependencies can affect the behavior it is trying to model.

## Make control flow mentally enumerable

The safety-critical tradition is useful here because restricted control flow reduces the number of possible execution stories a reader must maintain.
...
- functions whose important branching alternatives can be enumerated without extensive symbolic reasoning.

The goal is not to prove termination of arbitrary programs. It is to let the reader cheaply approximate what happens in representative cases and see where the exceptional/unbounded cases are.

Use cyclomatic complexity, cognitive complexity, nesting depth, or function size as **signals**, not universal aesthetic limits. Human-code-comprehension research shows that structural metrics correlate with understandability but are not accurate enough to substitute for actual comprehension tasks. A component with “good metrics” can still require a large mental model.

When a component becomes expensive to explain, simulate, or test, split the underlying decisions rather than mechanically extracting arbitrary blocks.
...
Hidden mutation is expensive because every reader must search for all possible writers before trusting a value. The same is true for implicit configuration, ambient request state, dynamic registries, or other non-local inputs.
...
This is not a mandate for functional programming. It is a locality mechanism: deterministic cores are cheaper to mentally simulate, test, replay, cache, inspect, and refactor; effectful edges make environmental assumptions explicit.
...
A weak reader should not need to inspect framework internals to discover that an apparently successful path silently discards failure.

## Names are retrieval and simulation infrastructure

Agents use symbol search, repository maps, ASTs, LSPs, and text retrieval. Names therefore affect both localization and the mental model built after retrieval.
...
- make intermediate state meaningful enough that a reader can follow transformations without re-deriving them.
...
For agent-only or agent-dominant code, every explanatory comment also consumes future context. It should carry decision-bearing information that cannot be recovered cheaply from the code.
...
Do **not** optimize formatting for guessed tokenizer tricks. There is currently no strong basis for custom minified or token-gamed source layouts outperforming conventional well-represented code, and they can damage tool support, diffs, training-distribution familiarity, and semantic comprehension.

Optimize **semantic work per token**, not fewest tokens at any cost.
...
Good tests also externalize part of the mental model: a future agent can inspect representative examples/invariants rather than derive every behavior from implementation alone.
...
## Enforce useful proxies mechanically
...
These mechanisms protect the conditions under which bounded mental modeling is likely to remain cheap. They do **not** prove that a language model actually understands the code.
...
| --- | --- |
| Mega-function / mega-module | Large context must be loaded; branches/state interact beyond a cheap mental model |
...
| Hidden global/service-locator state | Reader must search globally to learn inputs, dependencies, and mutation |
| Reflection/metaprogramming for ordinary control flow | Behavior cannot be reconstructed cheaply from the local source |
...
| Clever compressed expressions | Fewer tokens/lines but more semantic reasoning per token and worse diagnostics |
| Comments that narrate implementation | Duplicate source of truth that drifts and consumes context |
...
| Unbounded retries/queues/work | Runtime behavior and failure modes are difficult to mentally bound |
...
For agentic efficiency, the more useful questions are:

> **Can a weak reader predict this unit's behavior from a bounded local context?**
>
> **Does this boundary let the next change be understood and verified with less repository context?**

If an abstraction increases indirection without reducing mental-model size, change ripple, uncertainty, or test cost, it is probably negative leverage.

## Measure actual mental-model construction

Do not claim that smaller files or lower complexity save model tokens merely because they sound plausible.

The most direct test is to give a weaker model a deliberately bounded code slice and ask semantic questions with deterministic/independent ground truth, for example:

- which branches execute for these representative inputs?
- which values influence this result?
- which functions can be reached?
- what state is live/changed at this point?
- what side effects/errors can occur?
- does this loop/retry have a visible bound?
- which contract/invariant would this mutation violate?

Record accuracy **and how much extra context/reasoning the model needs before it can answer**.

This idea is supported by current semantic-understanding benchmarks: modern LLMs can generate code very well while remaining materially less reliable at foundational static semantic questions, especially more complex ones. Therefore the goal is not “a strong model can eventually understand it”; it is keeping ordinary units inside the reliable semantic envelope of cheaper/weaker readers.

For representative maintenance changes, also observe:
...
See [`../../experiments/agent-legibility.md`](../../experiments/agent-legibility.md) for the comparison and weak-reader mental-model protocol.

## Historical/evidence archive

The historical research behind these rules — punched-card/batch constraints, structured programming, Parnas information hiding, JPL/NASA safety-critical rules, real-time restrictions, flight-software architecture, modern testing practice, repository-level LLM retrieval, and agent-first repository experience — is preserved in [`../../references/agent-legible-code-history-2026-09.md`](../../references/agent-legible-code-history-2026-09.md).

Current evidence on language-model semantic understanding and the limits of static complexity proxies is preserved in [`../../references/agent-semantic-understanding-2026-09.md`](../../references/agent-semantic-understanding-2026-09.md).
