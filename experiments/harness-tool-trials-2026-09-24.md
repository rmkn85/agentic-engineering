...
## Compaction-threshold follow-up

The [Codex configuration reference](https://learn.chatgpt.com/docs/config-file/config-reference) documents a numeric `model_auto_compact_token_limit` and the `total` / `body_after_prefix` scope. A lower limit is a plausible response to late coordinator compaction, but no threshold has been promoted to the shared skill or project configuration. A nested CLI attempt against the public [coordination replay fixture](fixtures/coordination-replay/README.md) could authenticate and emit usage, yet every source command failed during sandbox startup before the task could be attempted. It was excluded as a behavioral baseline; no candidate run or cost comparison followed. Use a working host and accepted paired tasks before claiming a threshold improves total usage or outcome quality. The benchmark reports root compaction events when local session traces are available, but those markers alone do not distinguish manual from automatic triggers; [Codex hooks](https://learn.chatgpt.com/docs/hooks) can record that distinction prospectively.
