...
Add a launch-delivery check to the smallest relevant experiment: capture the actual launch cwd and the instruction sources delivered before work begins. If child guidance is absent, compare a short scoped workspace router that starts at the child root with the runtime's supported instruction-loading mechanism. A fresh, uncoached behavioral acceptance case must then exercise the rule and retain the actual trace plus acceptance result. Include at least one stale-session or parent-cwd case and one negative/held-out case where the child rule should not apply. A coached file read is a diagnostic condition, not startup-delivery evidence.
